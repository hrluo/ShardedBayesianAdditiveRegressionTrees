...
//--------------------------------------------------
// randomize random number generator in MPI so each node as a different seed/random sequence.
void brt::mpi_randrn(rn& gen)
{
   unsigned long ulstate=std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count() + (unsigned long)rank;
