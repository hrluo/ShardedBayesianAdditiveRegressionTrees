...
fit.a=openbt(x,y,pbd=c(0.7,0.0),ntreeh=1,numcut=20,tc=6,model="bart",modelname="branin-a")
fit.b=openbt(x,y,pbd=c(0.7,0.0),ntreeh=1,numcut=20,tc=6,model="bart",modelname="branin-b")

do_two_output = FALSE  # toggles between 2-output and 3-output mopareto examples

if (do_two_output) {  # 2-output mopareto example

  pf=mopareto.openbt(fit.a,fit.b,tc=6)  # quite slow, use many cores to speed it up

  # plot front
  par(mfrow=c(1,2))
  plot(0,0,xlim=c(-2,2),ylim=c(-2,2),type="n",xlab="y1",ylab="y2")
  for(vhr in pf) points(t(vhr$theta),pch=20)
  # plot set
  plot(0,0,xlim=c(0,1),ylim=c(0,1),type="n",xlab="x1",ylab="x2")
  for(vhr in pf) rect(vhr$a[1, ], vhr$a[2, ], vhr$b[1, ], vhr$b[2, ], col=gray(0.5,alpha=0.1), border=NA)

} else {  # 3-output mopareto example
  fit.c=openbt(x,y,pbd=c(0.7,0.0),ntreeh=1,numcut=20,tc=6,model="bart",modelname="branin-c")
  pf=mopareto.openbt(fit.a,fit.b,fit.c,tc=6)  # quite slow, use many cores to speed it up
  print(pf[[1]])

  # plot front
  library(rgl)
  # rglwidget()  # may need to uncomment this line
  plot3d(0,0,0,type="n",xlab="y1",ylab="y2",zlab="y3")
  for(vhr in pf) points3d(t(vhr$theta))
  # plot set
  plot(0,0,xlim=c(0,1),ylim=c(0,1),type="n",xlab="x1",ylab="x2")
  for(vhr in pf) rect(vhr$a[1, ], vhr$a[2, ], vhr$b[1, ], vhr$b[2, ], col=gray(0.5,alpha=0.1), border=NA)
}
