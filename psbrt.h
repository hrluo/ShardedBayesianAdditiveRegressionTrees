...
public:
   ...
   void settp(double alpha, double beta, bool sharding, size_t shardepth, double shardpsplit, bool randshard) { tp.alpha=alpha;tp.beta=beta;tp.sharding=sharding;tp.shardepth=shardepth;tp.shardpsplit=shardpsplit;tp.randshard=randshard; for(size_t j=0;j<m;j++) sb[j].settp(alpha,beta,sharding,shardepth,shardpsplit,randshard); }
