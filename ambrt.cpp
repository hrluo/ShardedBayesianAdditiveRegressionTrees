...
void ambrt::draw_mpislave(rn& gen)
{
  ...
  // If sharding enabled and randomize shard variable is true then generate new uniforms.
  // Note since the data is only on the slave nodes, this only has to run here, not in ambrt::draw().
  if(tp.randshard)
  {
    mpi_randrn(gen);
    for(size_t j=0;j<di->n;j+=di->p)
       di->x[j]=gen.uniform();
    mpi_resetrn(gen);
  }
