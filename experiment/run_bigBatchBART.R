...
X_file_check = paste0(summaryfile,'_',as.character(m),'_',as.character(args[1]),'_summary.csv')
...
datasummary <- as.data.frame( cbind( c(dim(fit.baseline$x.train)[1],dim(fit.sh0.ps1$x.train)[1],dim(fit.sh1.ps1$x.train)[1],dim(fit.sh2.ps1$x.train)[1],dim(fit.baseline.half$x.train)[1],dim(fit.baseline.quad$x.train)[1],dim(fit.sh0.ps05$x.train)[1],dim(fit.sh1.ps05$x.train)[1],dim(fit.sh2.ps05$x.train)[1]),rep(m,9),c(-1,0,1,2,-1,-1,0,1,2),c(-1,1,1,1,1,1,0.5,0.5,0.5),t(t(rmse)),t(t(coverage)),t(t(modeltime)) ) )
colnames(datasummary)<-c('n','m','shardepth','shardpsplit','RMSE','coverage','time')
print(datasummary)
if(length(args)>0){
	write.csv(datasummary, file=paste0(summaryfile,'_',as.character(m),'_',as.character(args[1]),'_summary.csv'),row.names = FALSE)
}else{
	write.csv(datasummary, file=paste0('branin_',as.character(m),'_',as.character(99),'_summary.csv'),row.names = FALSE)
}
